...
# --- YARDIMCI FONKSİYON: URL DOSTU METİN (SEO) ---
def slugify(text):
    text = text.lower()
    tr_map = str.maketrans("çğıöşü ", "cgiosu-")
    text = text.translate(tr_map)
    text = re.sub(r'[^a-z0-9-]', '', text) # Alfanumerik olmayanları temizle
    return text

# --- ASSET YÜKLEME ---
@st.cache_resource
def load_ml():
    return joblib.load("models/xgboost_model.joblib"), joblib.load("models/encoders.joblib")
...
if not filtered.empty:
    ...
    with col1:
        # --- SEO UYUMLU LİNK OLUŞTURMA ---
        ilce_slug = slugify(ev['district'])
        mahalle_slug = slugify(ev['neighborhood'])
        # Link Yapısı: https://www.hepsiemlak.com/istanbul-atasehir-yenisehir-satilik/daire/12345
